...
server.tool(
  "get-pr-release-status",
  "Return composite release readiness for a PR: CI check rollup, risk score, and releaseReady verdict. Requires GITHUB_TOKEN.",
  {
    owner: z.string().describe("GitHub repository owner"),
    repo: z.string().describe("GitHub repository name"),
...
server.tool(
  ...
  async ({ owner, repo, prNumber, commitSha, environment }): Promise<ToolReturn> => {
    ...
    let ciSummary = null;
    let releaseReady: boolean | undefined;
    let releaseReadyReasons: string[] | undefined;
