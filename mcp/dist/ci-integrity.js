 ...
 */
export const OR_TRUE_SUPPRESSION_ALLOWLIST = [
    {
        id: "trap-cleanup",
        category: "cleanup",
...
export function isAllowedOrTrueSuppression(line) {
...
export function detectCiIntegrity(files) {
    ...
    for (const file of files.filter((entry) => /\.(test|spec)\.(ts|tsx|js|jsx)$|__tests__\/|\.cy\.(ts|js)$/.test(entry.filename))) {
    ...
    for (const file of files) {
        ...
        if (!patch)
            continue;
        if (/^-\s*(branches|functions|lines|statements)\s*:\s*\d+/m.test(patch) &&
