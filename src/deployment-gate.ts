...
export function evaluateDeploymentGate(input: DeploymentGateInput): DeploymentGateResult {
  ...
  const ciLines =
    input.ciSummary && input.ciSummary.checks.length > 0
      ? `\n\n**CI:** ${
          input.ciSummary.checks
            .filter((c) => c.required)
