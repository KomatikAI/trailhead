...
- **Trigger-set and concurrency determinism for the labeled-trigger gate (train-37)** — `cli/src/generators.ts` single-sources the concurrency group suffix and the label job-`if` filter (`TRAILHEAD_GATE_CONCURRENCY_SUFFIX` / `TRAILHEAD_LABEL_JOB_IF`), and a new `workflow-trigger-parity.test.ts` fails the build the moment any shipped or documented copy (self-test, the reusable `release-ready.yml`, both example snippets, `docs/getting-started.md`) drifts from it — a silent divergence there is exactly how a consuming repo loses "a label event always yields exactly one authoritative evaluation" without anyone noticing. Separately, `createCheckRun` now tags every published check with the triggering workflow run's id (`external_id`) and, before publishing, checks whether a strictly newer run already published for the same head SHA — skipping with `superseded: true` if so. GitHub's `cancel-in-progress` concurrency group cancels by START order, not completion order, so an older run that wasn't killed before reaching this step could previously publish a stale conclusion after a newer run already published a fresh one, stranding the PR on the stale check (the train-37 failure). A superseded skip is reported in the Release Brief's `requiredCheck.message` as the expected last-write-wins outcome, not as a publication failure needing recovery guidance.
