export function parseVercelPayload(raw) {
    try {
        const payload = raw;
...
// ---------------------------------------------------------------------------
export function parseGenericPayload(raw) {
    try {
        const obj = raw;
...
async function rollbackVercel() {
    ...
    try {
        ...
        const listRes = await fetch(`https://api.vercel.com/v6/deployments?${params.toString()}`, {
        ...
        const promoteRes = await fetch(`https://api.vercel.com/v10/deployments/${previous.uid}/promote${promoteParams}`, {
            method: "POST",
            headers: {
...
// ---------------------------------------------------------------------------
async function rollbackGitHub(token, owner, repo, environment) {
    const headers = {
        Authorization: `Bearer ${token}`,
    ...
    try {
        const listRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/deployments?environment=${encodeURIComponent(environment)}&per_page=10`, { headers, signal: AbortSignal.timeout(10_000) });
        if (!listRes.ok) {
            return {
                triggered: false,
        ...
        for (const dep of deployments) {
            const statusRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/deployments/${dep.id}/statuses?per_page=1`, { headers, signal: AbortSignal.timeout(10_000) });
            if (!statusRes.ok)
                continue;
            const statuses = (await statusRes.json());
            if (statuses[0]?.state === "success") {
                const createRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/deployments`, {
                    method: "POST",
                    headers,
                    body: JSON.stringify({
                        ref: dep.sha,
...
// Entry point
// ---------------------------------------------------------------------------
export async function executeRollback(outcome, githubToken, repoFullName) {
    ...
    console.log(`[Trailhead] Deployment ${outcome.deploymentId} failed in ${outcome.environment} — initiating rollback`);
    ...
    return {
        triggered: false,
        strategy: "none",
        detail: "No rollback strategy available — set VERCEL_TOKEN/VERCEL_PROJECT_ID or provide GitHub token",
        timestamp: new Date().toISOString(),
    };
}
