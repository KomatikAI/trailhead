...
import { DEFAULT_SELF_CHECK_NAMES, evaluateRequiredChecks, normalizeCheckRuns, } from "./ci-core.js";
import { matchContext, resolveGateMode } from "./context-matcher.js";
import { evaluateDeploymentGate } from "./deployment-gate.js";
import { isInFreezeWindow } from "./risk-engine.js";
// ---------------------------------------------------------------------------
...
async function findPrForSha(token, owner, repo, sha) {
    try {
        const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/commits/${sha}/pulls`, { headers: ghHeaders(token) });
        if (!res.ok)
            return null;
        ...
        if (!summary)
            return null;
        const detailRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls/${summary.number}`, { headers: ghHeaders(token) });
        if (!detailRes.ok)
            return summary;
...
async function fetchChangedFilesFromApi(token, owner, repo, prNumber) {
    const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls/${prNumber}/files?per_page=300`, { headers: ghHeaders(token) });
    if (!res.ok)
        return [];
    return (await res.json());
}
async function fetchChangedFilesFromCommits(token, owner, repo, prNumber) {
    const commitsRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls/${prNumber}/commits?per_page=250`, { headers: ghHeaders(token) });
    if (!commitsRes.ok)
        return [];
    const commits = (await commitsRes.json());
    const fileMap = new Map();
    for (const commit of commits) {
        const detailRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/commits/${commit.sha}`, { headers: ghHeaders(token) });
        if (!detailRes.ok)
            continue;
...
async function fetchRepoConfig(token, owner, repo) {
    try {
        const headers = ghHeaders(token);
        let res = null;
        for (const path of [".trailhead.yml", ".deployguard.yml"]) {
            const candidate = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${path}`, { headers });
            if (candidate.ok) {
                res = candidate;
                break;
            }
...
async function fetchCheckRunsForRef(token, owner, repo, headSha) {
    const runs = [];
    let page = 1;
    while (true) {
        const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/commits/${headSha}/check-runs?per_page=100&page=${page}`, { headers: ghHeaders(token) });
        if (!res.ok)
            break;
        const data = (await res.json());
...
function resolveThresholds(repoConfig, environment, contextThresholds, defaults) {
    const envOverrides = repoConfig?.environments?.[environment];
    return {
        risk: contextThresholds?.risk ??
            envOverrides?.risk ??
            repoConfig?.thresholds?.risk ??
            defaults.risk,
        warn: contextThresholds?.warn ??
            envOverrides?.warn ??
            repoConfig?.thresholds?.warn ??
            defaults.warn,
    };
}
function resolveCiConfig(repoConfig, matchedContext) {
...
// Main handler
// ---------------------------------------------------------------------------
export async function handleDeploymentProtectionRule(payload, rawBody, signature) {
    ...
    const gateMode = resolveGateMode(repoConfig?.gate?.mode, schemaVersion, config.gateMode);
    const matched = pr
        ...
        : null;
    const thresholds = resolveThresholds(repoConfig, payload.environment, matched?.context.thresholds, { risk: config.riskThreshold, warn: config.warnThreshold });
    let ciSummary = null;
    if (gateMode !== "risk-only" && pr) {
        const ciConfig = resolveCiConfig(repoConfig, matched);
        const allChecks = await fetchCheckRunsForRef(token, owner, repo, payload.deployment.sha);
    ...
    const state = result.approved ? "approved" : "rejected";
