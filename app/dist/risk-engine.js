...
// Weighted average
// ---------------------------------------------------------------------------
export function weightedAverageScores(factors, overrides) {
    if (factors.length === 0)
        return 0;
    let totalWeight = 0;
    let weightedSum = 0;
...
// ---------------------------------------------------------------------------
export function computeRiskScore(files, config) {
    if (files.length === 0) {
...
// ---------------------------------------------------------------------------
export function computeSecurityFactor(alerts) {
    if (alerts.total === 0)
        return null;
...
// ---------------------------------------------------------------------------
export function computeDeploymentHistoryFactor(outcomes) {
...
export function isInFreezeWindow(freezes, now) {
    if (freezes.length === 0)
        return { frozen: false };
...
export function decideGate(riskScore, healthScore, blockThreshold, warnThreshold) {
...
// Rollback detection
// ---------------------------------------------------------------------------
export function isRollback(prTitle) {
